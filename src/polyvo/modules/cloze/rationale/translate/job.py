"""
`Job` sozlesmesinin cloze ipucu/aciklama cevirisi karsiligi. Motorun
bekledigi uc metodu baglar, baska is yapmaz.
"""
...
from polyvo.modules.cloze.rationale.translate import prompt as prompt_mod
from polyvo.modules.cloze.rationale.translate import qa as qa_mod
from polyvo.modules.cloze.rationale.translate import units


class ClozeRationaleTranslationJob(Job):
    """Onaylanmis ipucu/aciklama paketini bir L1'e ceviren is."""

    family = "cloze"
    kind = "cloze_rationale_l1"
    command = "cloze rationale-translate"
    ...
    def load_units(self, ctx: JobContext) -> list[Unit]:
        """Onayli ipucu/aciklama paketi olan anlamlari yukler, varyant
        anahtarini uygular. `ctx.variant` bu kosuda HER ZAMAN `ctx.l1`dir."""
