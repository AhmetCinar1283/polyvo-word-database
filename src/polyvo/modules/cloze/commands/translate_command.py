"""
`polyvo cloze translate` — onaylanmis cloze cumlelerini bir ana dile cevirir.

`generate_command.py`nin ikizidir; tek fark `JobContext.variant = l1`: bu
kosu ayri bir anahtar ailesinde yasar, Ingilizce paketin anahtarini EZMEZ.

Siklar CEVRILMEZ — cevrilen sey cumledir (V2-IS-4 §14).
"""
...
def cmd_translate(args: argparse.Namespace) -> int:
    """Cloze ceviri kosusunu calistirir; cikis kodunu doner."""
    ...
    try:
        result = engine_run.run(ClozeTranslationJob(), ctx, provider=provider,
                                store=store, **cli_args.run_kwargs(args))
    except LLMUnavailable as exc:
        llm_cli.fail(exc, "cloze translate")
        return 1
    except KeyboardInterrupt:
        print("\n[cloze translate] kullanici durdurdu — yazilanlar korundu.")
        return 130
    finally:
        store.close()
