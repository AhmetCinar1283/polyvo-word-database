"""
`polyvo cloze rationale-translate` — onaylanmis ipucu/aciklama paketini bir
ana dile cevirir.

`translate_command.py`nin ikizidir; tek fark girdi: cumle degil ipucu +
sik aciklamasi cevrilir. `sense_cloze_hint`/`sense_cloze_option_reason`
tablolarina dokunulmaz.
"""
...
def add_rationale_translate_args(parser: argparse.ArgumentParser) -> None:
...
def cmd_rationale_translate(args: argparse.Namespace) -> int:
    """Ipucu/aciklama ceviri kosusunu calistirir; cikis kodunu doner."""
    ...
    try:
        result = engine_run.run(ClozeRationaleTranslationJob(), ctx,
                                provider=provider, store=store,
                                **cli_args.run_kwargs(args))
    except LLMUnavailable as exc:
        llm_cli.fail(exc, "cloze rationale-translate")
        return 1
    except KeyboardInterrupt:
        print("\n[cloze rationale-translate] kullanici durdurdu — "
             "yazilanlar korundu.")
        return 130
    finally:
        store.close()
